...
if(body) {
    switch( !0) {
        case/pgc\/season\/app\/related\/recommend\?/.test($request.url):try {
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/skin\?/.test($request.url):try {
        ...
        case/^https:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\?/.test($request.url):try {
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/v2\/feed\/index\/story\?/.test($request.url):try {
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/v\d\/account\/teenagers\/status\?/.test($request.url):try {
        ...
        case/^https?:\/\/app\.bilibili\.com\/x\/resource\/show\/tab/.test($request.url):try {
...
case/^https?:\/\/app\.bilibili\.com\/x\/v2\/account\/mine/.test($request.url):try {
    ...
    v.data.sections_v2.forEach((t, i)=> {
            let a=t.items.filter(t=>$.has(t.id)); v.data.sections_v2[i].items=a, v.data.sections_v2[i].button= {}

            , delete v.data.sections_v2[i].be_up_title, delete v.data.sections_v2[i].tip_icon, delete v.data.sections_v2[i].tip_title, ("创作中心"==v.data.sections_v2[i].title||"創作中心"==v.data.sections_v2[i].title)&&(delete v.data.sections_v2[i].title, delete v.data.sections_v2[i].type)
...
case/^https?:\/\/api\.live\.bilibili\.com\/xlive\/app-room\/v1\/index\/getInfoByRoom/.test($request.url):try {
...
case/^https?:\/\/app\.bilibili\.com\/x\/resource\/top\/activity/.test($request.url):try {
...
case/ecommerce-user\/get_shopping_info\?/.test($request.url):try {
...
case/^https?:\/\/app\.bilibili\.com\/x\/v2\/search\/square/.test($request.url):try {
    let j=JSON.parse(body);

    j.data= {
        type: "history", title:"搜索历史", search_hotword_revision:2
    }
...
case/https?:\/\/app\.bilibili\.com\/x\/v2\/account\/myinfo\?/.test($request.url):try {
...
case/pgc\/page\/(bangumi|cinema\/tab\?)/.test($request.url):try {
...
case/^https:\/\/app\.bilibili\.com\/x\/v2\/splash\/list/.test($request.url):try {
    let R=JSON.parse(body);
    if(R.data&&R.data.list)for(let S of R.data.list)S.duration=0,
...
case/^https:\/\/api\.live\.bilibili\.com\/xlive\/app-interface\/v2\/index\/feed/.test($request.url):try {
    let A=JSON.parse(body);
